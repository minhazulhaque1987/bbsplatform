...
var CROP_NOTIF_KEY   = 'bbs_crop_notif_v1';  // 'on' | 'off'
var CROP_NOTIFIED_KEY= 'bbs_crop_notified_v1'; // { "u2d_2026_3_15": "2026-03-14" }
...
function pickModeFor(user) {
  if (!user) return null;
  // Accept both camelCase and snake_case variants stored by different signup versions
  const type = user.officeType || user.office_type || '';
  switch (type) {
...
function renderCropView() {
  ...
  if (u) {
    ...
    // Accept multiple field-name variants saved by different signup code versions
    if (ptEl) ptEl.textContent = u.post || u.designation || u.position || '—';
    if (ofEl) ofEl.textContent = u.office || u.workplace || u.officeTitle || '—';
    if (tpEl) {
      tpEl.textContent = u.officeType || u.office_type || (u.role === 'admin' ? 'এডমিন' : 'অজানা');
      tpEl.style.display = '';
    }
    if (avEl) {
      if (u.photo) {
