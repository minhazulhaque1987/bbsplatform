...
async function doSignup() {
  ...
  // পদবি ভ্যালিডেশন: তালিকা থেকে অবশ্যই একটি পদবি সিলেক্ট করতে হবে
  const selPosition = (typeof getSelectedPosition === 'function') ? getSelectedPosition() : null;
  if(!post) {
    setErr('s-post','s-post-e','পদবি দিন'); ok=false;
  } else if(!selPosition) {
    setErr('s-post','s-post-e','তালিকা থেকে একটি পদবি নির্বাচন করুন'); ok=false;
  } else {
    setOk('s-post');
  }
  ...
  if(existingEmailUser){ setErr('s-email','s-email-e','এই ইমেইলে অ্যাকাউন্ট আছে'); return }

  const existingPhoneUser = await getUserByPhone(phone);
  if(existingPhoneUser){ setErr('s-phone','s-phone-e','এই মোবাইলে অ্যাকাউন্ট আছে'); return }
  ...
  if (!addedUser) {
    setErr('s-name','s-name-e','অ্যাকাউন্ট তৈরি করতে সমস্যা হয়েছে। আবার চেষ্টা করুন।');
    return;
  }
