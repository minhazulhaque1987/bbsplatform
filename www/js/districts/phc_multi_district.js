...
function renderPHCDistrictSelector() {
  ...
  const available = PHC_DISTRICT_REGISTRY.filter(d => d.available);
  const coming    = PHC_DISTRICT_REGISTRY.filter(d => !d.available);
  ...
  const availCards = available.map(d => `
    <div class="phc-dist-sel-card" onclick="loadPHCDistrict('${d.id}')"
      ...
      <div class="phc-dsc-stats">
        ...
        <span class="phc-dsc-type">${d.report_type === 'community' ? 'কমিউনিটি রিপোর্ট' : 'জেলা রিপোর্ট'}</span>
      </div>
    </div>`).join('');
...
function loadPHCDistrict(districtId) {
  const reg = PHC_DISTRICT_REGISTRY.find(d => d.id === districtId);
  if (!reg || !reg.available) return;

  phcActiveDistrict = reg;
  phcActiveData     = window[reg.data_obj];
  phcActiveUpazilas = window[reg.upazilas_obj];

  if (!phcActiveData || !phcActiveUpazilas) {
    if (typeof toast === 'function') toast('ডেটা লোড হয়নি', 'warn');
    return;
  }

  // Reset tabs to search
  phcCurrentTab    = 'search';
  phcSearchQuery   = '';
  phcSelectedCat   = 'সব';
  phcCompareList   = [];
  ...
  const cards = phcActiveUpazilas.map(u => `
    <div class="phc-upz-card" onclick="openPHCUpazilaDistrict(${u.id})"
      ...
      <div class="phc-upz-card-top" style="background:linear-gradient(135deg,${phcActiveDistrict.color},${phcActiveDistrict.color}dd)">
        ...
        <div class="phc-upz-card-sub">${u.name_en}${u.type==='city_corporation'?' (সিটি কর্পোরেশন)':''}</div>
      </div>
      <div class="phc-upz-kpis">
        ...
        <div class="phc-kpi"><span class="phc-kpi-val">${bnN(u.literacy_7plus_2022,1)}%</span><span class="phc-kpi-lbl">সাক্ষরতা</span></div>
        <div class="phc-kpi"><span class="phc-kpi-val">${u.internet_pct!=null?bnN(u.internet_pct,1)+'%':bnN(u.sdg_internet,1)+'%'}</span><span class="phc-kpi-lbl">ইন্টারনেট</span></div>
        <div class="phc-kpi"><span class="phc-kpi-val">${bnN(u.electricity_grid_pct,1)}%</span><span class="phc-kpi-lbl">গ্রিড বিদ্যুৎ</span></div>
      </div>
      <div class="phc-upz-bar-row">
        ${miniBar('NEET',u.neet_total_pct,'#C62828')}
        ${miniBar('নিরাপদ টয়লেট',u.toilet_safe_flush_pct,'#00695C')}
        ${miniBar('পাকা ঘর',u.dwelling_pucca_pct,'#6A1B9A')}
      </div>
      <div class="phc-upz-card-foot">
        <span>👥 ${bnInt(u.pop_total_2022)}</span>
...
function renderPHCUpazilaDetailDistrict() {
  ...
  if (hName) hName.textContent = u.name_bn + (u.type==='city_corporation' ? ' সিটি কর্পোরেশন' : ' উপজেলা');
  ...
  const sections = [
    {
      icon:'👥', title:'জনসংখ্যা', rows:[
        ...
        ['হিজড়া', u.pop_hijra_2022!=null?bnN(u.pop_hijra_2022):'—'],
        ...
        ['লিঙ্গানুপাত (প্রতি ১০০ মহিলায় পুরুষ)', u.sex_ratio_2022!=null?bnN(u.sex_ratio_2022,2):'—'],
    ...
    },
    {
      icon:'📚', title:'সাক্ষরতা', rows:[
        ['সাক্ষরতার হার, ৭+ বছর (মোট)', bnPct(u.literacy_7plus_2022)],
        ['সাক্ষরতার হার, ৭+ বছর (পুরুষ)', bnPct(u.literacy_7plus_male)],
        ['সাক্ষরতার হার, ৭+ বছর (মহিলা)', bnPct(u.literacy_7plus_female)],
        ['সাক্ষরতার হার, ১৫+ বছর (মোট)', bnPct(u.literacy_15plus_2022)],
        ['সাক্ষরতার হার, ১৫+ বছর (পুরুষ)', bnPct(u.literacy_15plus_male)],
        ['সাক্ষরতার হার, ১৫+ বছর (মহিলা)', bnPct(u.literacy_15plus_female)],
      ]
    },
    {
      icon:'🏠', title:'খানা ও বাসগৃহ', rows:[
        ['মোট সাধারণ খানা', bnInt(u.hh_general_2022)],
    ...
    },
    {
      icon:'🎯', title:'NEET (শিক্ষা ও কর্মহীন যুব)', rows:[
        ...
        ['NEET (পুরুষ)', u.neet_male_pct!=null?bnPct(u.neet_male_pct):'—'],
        ['NEET (মহিলা)', u.neet_female_pct!=null?bnPct(u.neet_female_pct):'—'],
    ...
    },
    {
      icon:'📱', title:'প্রযুক্তি ও আর্থিক অন্তর্ভুক্তি', rows:[
        ['মোবাইল ফোন ব্যবহারকারী', u.mobile_phone_pct!=null?bnPct(u.mobile_phone_pct):'—'],
        ['ইন্টারনেট ব্যবহারকারী', u.internet_pct!=null?bnPct(u.internet_pct):'—'],
        ['আর্থিক অ্যাকাউন্টধারী', u.financial_account_pct!=null?bnPct(u.financial_account_pct):'—'],
        ['মোবাইল ব্যাংকিং', u.mobile_banking_pct!=null?bnPct(u.mobile_banking_pct):'—'],
      ]
    },
    {
...
function renderPHCDistrictView() {
  ...
  content.innerHTML = `
    <div class="phc-district-banner" style="background:linear-gradient(135deg,${phcActiveDistrict.color},${phcActiveDistrict.color}cc)">
      ...
      <div class="phc-dist-sub">জনশুমারি ও গৃহগণনা ২০২২ — ${d.report_type==='district'?'জেলা রিপোর্ট':'কমিউনিটি রিপোর্ট'}</div>
      <div class="phc-dist-kpis">
        <div class="phc-dist-kpi"><div class="phc-dist-kpi-val">${Number(d.pop_total_2022).toLocaleString('bn-BD')}</div><div class="phc-dist-kpi-lbl">মোট জনসংখ্যা</div></div>
